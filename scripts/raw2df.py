...
out_data = pd.DataFrame(columns=['bench', 'power', 'sched', 'id', 'perf', 'energy', 'runtime', 'path'])
tmp_list = []
...
for d in in_data.iterrows():
    ...
    elif sched in [ '5.4-fdp-percpu' ]:
        sched = 'fdp-percpu-{}'.format(50 if lp == 'n' else lp)
    ...
    energy = d[1]['energy']
    runtime = d[1]['usr_bin_time']

    tmp_list.append({ 'bench': bench, 'sched': sched, 'power': power, 'id': run_id, 'perf': perf, 'energy': energy, 'runtime': runtime, 'path': d[1]['fname'] })
